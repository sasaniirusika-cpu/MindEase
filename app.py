...
:root {
    --primary: #02C39A;
    --primary-dark: #028090;
    --primary-glow: rgba(2, 195, 154, 0.2);
    --primary-border: rgba(2, 195, 154, 0.3);
    --bg-base: #0A0F1A;
    --bg-card: #111827;
    --bg-card2: #1A2235;
    --bg-input: #0D1520;
    ...
    --shadow-teal: 0 8px 32px rgba(2,195,154,0.15);
}
...
.main-header::before {
    ...
    width: 200%;
    height: 200%;
    background: radial-gradient(ellipse at center, rgba(2,195,154,0.06) 0%, transparent 60%);
    pointer-events: none;
}
.main-header h1 {
    color: var(--text-primary) !important;
...
.mcard:hover { border-color: var(--primary-border); }
.mcard-plain {
...
.mcard-accent {
    background: linear-gradient(135deg, rgba(2,195,154,0.08), rgba(2,128,144,0.05));
    border: 1px solid var(--primary-border);
    border-radius: 16px;
    padding: 1.25rem 1.5rem;
    margin: 0.6rem 0;
...
.badge-earned:hover { box-shadow: var(--shadow-teal); }
.badge-locked {
    background: var(--bg-card);
    border: 1px solid var(--border);
    border-radius: 10px;
...
.stButton > button:hover {
    ...
    box-shadow: 0 6px 20px rgba(2,195,154,0.35) !important;
}
...
section[data-testid="stSidebar"] [data-testid="stButton-logout"] > button:hover {
    background: rgba(2,195,154,0.18) !important;
    transform: none !important;
}
...
if not st.session_state.logged_in:
    ...
    <div style='text-align:center; padding:3rem 0 1.5rem;'>
        ...
        <h1 style='color:#F0F4FF; font-size:2.2rem; font-weight:700; margin:0; letter-spacing:-0.5px;'>MindEase</h1>
        <p style='color:#8899BB; font-size:0.95rem; margin:0.5rem 0 0 0; font-weight:300; letter-spacing:0.5px;'>YOUR PERSONAL WELLNESS COMPANION</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        border-radius:20px; padding:2rem 2rem 1.5rem; box-shadow:0 20px 60px rgba(0,0,0,0.5);'>
        <p style='color:#8899BB; font-size:0.75rem; letter-spacing:1.5px; text-transform:uppercase;
...
MOOD_COLORS = {
    "Happy":    {"color": "#F4C542", "bg": "rgba(244,197,66,0.08)",   "label": "Happy",   "message": "You are radiating great energy today. Keep it going."},
    "Okay":     {"color": "#6BBBF7", "bg": "rgba(107,187,247,0.08)", "label": "Calm",    "message": "A steady, balanced day. That is perfectly fine."},
    "Sad":      {"color": "#7B9CDE", "bg": "rgba(123,156,222,0.08)", "label": "Low",     "message": "It is okay to feel sad. Be gentle with yourself today."},
    "Stressed": {"color": "#F07070", "bg": "rgba(240,112,112,0.08)", "label": "Stressed","message": "Take a slow breath. You can work through this."},
    "Angry":    {"color": "#E8724A", "bg": "rgba(232,114,74,0.08)",  "label": "Angry",   "message": "Your feelings are valid. Try the breathing exercise."},
}
...
with st.sidebar:
    ...
    <div style='padding:1.5rem 0.5rem 1rem;'>
        <div style='font-size:1.1rem; font-weight:700; color:#F0F4FF; letter-spacing:-0.3px;'>MindEase</div>
        <div style='font-size:0.7rem; color:#8899BB; letter-spacing:1px; text-transform:uppercase; margin-top:0.2rem;'>Wellness Companion</div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown(f"""
    <div style='background:#1A2235; border:1px solid rgba(255,255,255,0.07);
    border-radius:12px; padding:0.9rem 1rem; margin-bottom:0.5rem;'>
        <div style='font-weight:600; font-size:0.9rem; color:#F0F4FF;'>{name}</div>
        <div style='font-size:0.72rem; color:#8899BB; margin-top:0.15rem;'>{email}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.divider()
    st.markdown("<p style='font-size:0.68rem; color:#8899BB; letter-spacing:1.2px; text-transform:uppercase; margin:0.3rem 0 0.5rem;'>MUSIC</p>", unsafe_allow_html=True)
    ...
    if st.session_state.music_playing and st.session_state.selected_music:
        ...
        <div style='font-size:0.72rem; color:#8899BB; margin-top:0.3rem;'>
        Now playing — {st.session_state.selected_music}
        </div>
        """, unsafe_allow_html=True)

    st.divider()
    st.markdown("<p style='font-size:0.68rem; color:#8899BB; letter-spacing:1.2px; text-transform:uppercase; margin:0.3rem 0 0.5rem;'>CONVERSATIONS</p>", unsafe_allow_html=True)
    ...
    if chat_history:
        ...
        for conv in chat_history[:6]:
            ...
            <div class='chat-history-item'>
                <div style='font-weight:500; color:#F0F4FF; font-size:0.78rem;'>{date}</div>
                <div style='color:#8899BB; font-size:0.73rem; margin-top:0.1rem;'>{preview}</div>
            </div>
            """, unsafe_allow_html=True)
    else:
        st.markdown("<p style='font-size:0.78rem; color:#8899BB;'>No conversations yet.</p>", unsafe_allow_html=True)

    st.divider()
    st.markdown("<p style='font-size:0.68rem; color:#8899BB; letter-spacing:1.2px; text-transform:uppercase; margin:0.3rem 0 0.5rem;'>NAVIGATE</p>", unsafe_allow_html=True)
    ...
    section[data-testid="stSidebar"] [data-testid="stButton-nav_{active_key}"] > button {{
        ...
        border: 1px solid rgba(2,195,154,0.25) !important;
    }}
...
if page == "💬 Chat":
    ...
    user_input = st.chat_input(f"Talk to MindEase...")
    if user_input:
        with chat_container:
...
if page == "😊 Mood":
    ...
    border-radius:16px; padding:2rem; margin:0.8rem 0;'>
        <div style='font-size:0.7rem; letter-spacing:2px; text-transform:uppercase;
        color:{md["color"]}; font-weight:600; margin-bottom:0.5rem;'>{md["label"]}</div>
        <div style='width:60px;height:60px;border-radius:50%;background:{md["color"]};
        margin:0 auto 0.8rem; opacity:0.85;'></div>
        <div style='font-size:0.9rem; color:#8899BB; font-weight:300;'>{md["message"]}</div>
    </div>
    """, unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        ...
        if st.button("Get a Message", use_container_width=True):
            with st.spinner(""):
                try:
                    ...
                    st.markdown(f'<div class="mcard-accent">{msg}</div>', unsafe_allow_html=True)
                except Exception as e:
                    st.error(str(e))
    ...
    if mood_data:
        ...
        st.markdown('<div class="mcard"><p style="color:#8899BB; font-size:0.88rem; margin:0;">No mood logs yet. Log your first mood above.</p></div>', unsafe_allow_html=True)

# ══ PAGE 3 — JOURNAL ═══════════════════════════════════════════
if page == "📝 Journal":
    st.markdown(f'<div class="section-title">Journal — {name}</div>', unsafe_allow_html=True)
    st.markdown('<p style="color:#8899BB; font-size:0.85rem; margin:0 0 0.8rem;">Your private space. Write freely.</p>', unsafe_allow_html=True)
    ...
    if is_morning:
        with st.spinner(""):
            try:
                ...
                st.markdown(f'<div class="mcard-accent">{gm}</div>', unsafe_allow_html=True)
            except:
                st.markdown(f'<div class="mcard">Good morning, {name}. Today is a new beginning.</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="mcard"><p style="color:#8899BB; font-size:0.88rem; margin:0;">Morning section is active from 5 AM to 12 PM. See you tomorrow morning, {name}.</p></div>', unsafe_allow_html=True)
    ...
    if st.button("Get AI Suggestions", use_container_width=True):
        with st.spinner(""):
            try:
                ...
                st.markdown(f'<div class="mcard-accent">{suggestions}</div>', unsafe_allow_html=True)
            except Exception as e:
                st.error(str(e))
    ...
    if ex_type == "Breathing Exercise":
        ...
        if st.button("Begin", use_container_width=True):
            for i in range(rounds):
                ...
                for label, grad, toast_msg, secs in [
                    ...
                    ("Breathe Out","#028090,#0A0F1A","Breathe OUT...",6),
                ]:
                    bp.markdown(f'<div style="width:180px;height:180px;border-radius:50%;background:linear-gradient(135deg,{grad});display:flex;align-items:center;justify-content:center;margin:auto;color:white;font-size:1rem;font-weight:600;letter-spacing:0.5px;">{label}</div>', unsafe_allow_html=True)
                    st.toast(toast_msg)
                    time.sleep(secs)
            bp.markdown('<div style="width:180px;height:180px;border-radius:50%;background:linear-gradient(135deg,#02C39A,#028090);display:flex;align-items:center;justify-content:center;margin:auto;color:white;font-size:1rem;font-weight:600;">Complete</div>', unsafe_allow_html=True)
        ...
        if st.button("Begin Meditation", use_container_width=True):
            for i,(title,instruction,secs) in enumerate(MEDS[med_type]):
                st.markdown(f'<div class="mcard"><strong style="color:#02C39A; font-size:0.72rem; letter-spacing:1px; text-transform:uppercase;">Step {i+1} — {title}</strong><br><br><span style="color:#8899BB; font-weight:300;">{instruction}</span></div>', unsafe_allow_html=True)
                time.sleep(secs)
...
if page == "🎯 Affirmations":
    ...
    for a in AFFIRMATIONS:
        st.markdown(f'<div class="mcard" style="padding:0.75rem 1.2rem; margin:0.3rem 0;"><p style="margin:0; color:#F0F4FF; font-size:0.88rem; font-weight:300;">{a}</p></div>', unsafe_allow_html=True)
...
if page == "🧠 Assessment":
    ...
    st.markdown('<p style="color:#8899BB; font-size:0.85rem; margin:0 0 1rem;">Answer 8 questions honestly. This is not a medical diagnosis — it is a personal check in tool.</p>', unsafe_allow_html=True)
    qs = [
    ...
    if st.button("View Results", use_container_width=True):
        ...
        for t in tips:
            st.markdown(f'<div class="mcard" style="padding:0.6rem 1rem; margin:0.25rem 0;"><p style="margin:0; color:#F0F4FF; font-size:0.85rem;">{t}</p></div>', unsafe_allow_html=True)
        st.markdown('<div class="section-title">Watch</div>', unsafe_allow_html=True)
        for title, url in vids:
            st.markdown(f'<p style="color:#8899BB; font-size:0.82rem; margin:0.5rem 0 0.2rem;">{title}</p>', unsafe_allow_html=True)
            st.markdown(f'<iframe width="100%" height="180" src="{url}" frameborder="0" allowfullscreen style="border-radius:12px;"></iframe>', unsafe_allow_html=True)
        st.markdown('<p style="color:#8899BB; font-size:0.75rem; margin-top:1rem;">Not a medical diagnosis. Please see a professional if you are struggling.</p>', unsafe_allow_html=True)
...
if page == "🏆 Achievements":
    ...
    with c1:
        st.markdown(f'<div class="score-box"><div style="font-size:0.65rem; letter-spacing:1.5px; text-transform:uppercase; color:#8899BB; margin-bottom:0.5rem;">STREAK</div><div style="font-size:2.2rem; font-weight:700; color:#02C39A;">{streak}</div><div style="font-size:0.75rem; color:#8899BB;">days</div></div>', unsafe_allow_html=True)
    with c2:
        sc = 0
        mood_data = db_fetch("mood_logs", email)
        if mood_data:
        ...
        sc += min(streak*2,20)
        sc_col = "#02C39A" if sc>=80 else "#F4A261" if sc>=60 else "#E9C46A" if sc>=40 else "#E06C6C"
        st.markdown(f'<div class="score-box" style="border-color:{sc_col}44;"><div style="font-size:0.65rem; letter-spacing:1.5px; text-transform:uppercase; color:#8899BB; margin-bottom:0.5rem;">WELLNESS</div><div style="font-size:2.2rem; font-weight:700; color:{sc_col};">{sc}</div><div style="font-size:0.75rem; color:#8899BB;">out of 100</div></div>', unsafe_allow_html=True)
    with c3:
        earned_count = sum(1 for b in BADGES if b["fn"](mood_count,sleep_count,journal_count,streak))
        st.markdown(f'<div class="score-box"><div style="font-size:0.65rem; letter-spacing:1.5px; text-transform:uppercase; color:#8899BB; margin-bottom:0.5rem;">BADGES</div><div style="font-size:2.2rem; font-weight:700; color:#02C39A;">{earned_count}</div><div style="font-size:0.75rem; color:#8899BB;">of {len(BADGES)}</div></div>', unsafe_allow_html=True)
    ...
    if earned:
        st.markdown('<p style="color:#02C39A; font-size:0.72rem; letter-spacing:1px; text-transform:uppercase; margin:0 0 0.5rem;">EARNED</p>', unsafe_allow_html=True)
        cols = st.columns(3)
        for i, b in enumerate(earned):
            with cols[i%3]:
                st.markdown(f'<div class="badge-earned"><div style="font-size:1.4rem; color:#02C39A;">{b["icon"]}</div><div style="color:#F0F4FF; font-weight:600; font-size:0.82rem; margin:0.3rem 0 0.1rem;">{b["name"]}</div><div style="color:#8899BB; font-size:0.72rem;">{b["desc"]}</div></div>', unsafe_allow_html=True)
    if unearned:
        st.markdown('<p style="color:#8899BB; font-size:0.72rem; letter-spacing:1px; text-transform:uppercase; margin:0.8rem 0 0.5rem;">LOCKED</p>', unsafe_allow_html=True)
        for b in unearned:
            st.markdown(f'<div class="badge-locked"><span style="color:#8899BB; font-size:0.82rem;">{b["name"]}</span> <span style="color:#8899BB; font-size:0.75rem; opacity:0.6;">— {b["desc"]}</span></div>', unsafe_allow_html=True)
...
if page == "📈 Progress":
    ...
    try:
        ...
        else: st.markdown(f'<div class="mcard"><p style="color:#8899BB; font-size:0.88rem; margin:0;">No mood logs in the last {days} days.</p></div>', unsafe_allow_html=True)
    except: st.markdown('<div class="mcard"><p style="color:#8899BB; font-size:0.88rem; margin:0;">No mood data yet.</p></div>', unsafe_allow_html=True)
    ...
    try:
        ...
        else: st.markdown(f'<div class="mcard"><p style="color:#8899BB; font-size:0.88rem; margin:0;">No sleep logs in the last {days} days.</p></div>', unsafe_allow_html=True)
    except: st.markdown('<div class="mcard"><p style="color:#8899BB; font-size:0.88rem; margin:0;">No sleep data yet.</p></div>', unsafe_allow_html=True)
    ...
    except: st.markdown('<div class="mcard"><p style="color:#8899BB; font-size:0.88rem; margin:0;">No journal data yet.</p></div>', unsafe_allow_html=True)
